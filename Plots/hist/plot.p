set term postscript eps enhanced color 24
#set term epslatex 18
set view equal xy
#set size 1,1.1
set output 'output.eps'

set boxwidth 0.8 absolute
set style fill   pattern 1.00 border lt -1
set key samplen 1.5 spacing .5 font ",8"
set key horizontal inside left top nobox
set style histogram clustered gap 2.5 title textcolor lt -1
set datafile missing '-'
set style data histograms
set xtics border in scale 0,0 nomirror rotate by -45
#set xtics border in scale 0,0 nomirror rotate by 0
set xtics  norangelimit
set xtics   ()

#set xtics nomirror offset 0,graph 0.02
#set ytics border in scale 0,0 mirror norotate  offset character 0, 0, 0 autofreq 
#set title "Query Time Comparison"

set key spacing 0.8
set key font "Helvetica,33,bold"
set xlabel font "Helvetica,42,bold"
set ylabel font "Helvetica,48,bold"
set xtics font "Helvetica,39,bold"
set ytics font "Helvetica,34,bold"
set title font "Helvetica,42,bold"

set ylabel "Interpretability" 
set xlabel "Data set" 
set ylabel offset 0,0


set yrange [ 0 : 1.2 ] noreverse nowriteback
#set ytics 100000

plot 'data.txt' using ($2):xtic(1) ti col lw 4 lc 1 lt 1, '' u ($3) ti col lw 4 lc 2 lt 1,  '' u ($4) ti col lw 4 lc 3 lt 1,  '' u ($5) ti col lw 4 lc 4 lt 1
